# routes/roadmap.py  —  updated for Supabase stack
import uuid
import json
...
async def generate_roadmap(request: GenerateRoadmapRequest):
    """
    1. Call Gemini → get roadmap JSON
    2. Persist roadmap + nodes to Supabase PostgreSQL
    3. Cache full response in Supabase Redis (Upstash)
    ...
    for node in nodes:
        await db.insert_node(roadmap_id=roadmap_id, node=node)

    # ── Step 3: Build + cache response ───────────────────────────────────────
    response_data = {
        "roadmap_id":  roadmap_id,
        "user_id":     request.user_id,
        "title":       raw["title"],
...
# List all roadmaps for a user (dashboard)
# ─────────────────────────────────────────────────────────────────────────────

@router.get("/user/{user_id}")
async def get_user_roadmaps(user_id: str):
    """Returns all roadmaps created by this user, newest first."""
    roadmaps = await db.get_user_roadmaps(user_id)
